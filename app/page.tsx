    ...
    <>
      ...
      <main className="min-h-screen bg-background text-foreground">
        <section id="top" className="relative flex min-h-[780px] items-end overflow-hidden border-b border-white/10 px-6 pb-20 pt-20 lg:min-h-[880px] lg:px-10 lg:pb-28 lg:pt-24">
          ...
          <div className="relative mx-auto w-full max-w-7xl">
            <div className="max-w-4xl">
              <p className="mb-7 flex items-center gap-3 text-xs font-semibold uppercase tracking-[0.28em] text-accent">
                <span className="h-px w-8 bg-accent" />Systems for what&apos;s next
              </p>
              ...
              <p className="mt-8 max-w-xl text-pretty text-lg leading-7 text-muted-foreground sm:text-xl">
            ...
            <div className="mt-20 grid w-full grid-cols-3 gap-8 border-t border-white/20 pt-5 lg:gap-16">
              {stats.map(stat => (
        ...
        <section className="relative overflow-hidden border-b border-white/10 bg-surface px-6 py-8 lg:px-10">
          ...
          <div className="relative mx-auto flex max-w-7xl flex-wrap items-center justify-between gap-6 text-xs font-semibold uppercase tracking-[0.18em] text-muted-foreground">
        ...
        <section className="mx-auto max-w-7xl px-6 py-28 lg:px-10 lg:py-36">
          ...
          <div className="mt-14 grid gap-5 md:grid-cols-3">
            {teasers.map(teaser => (
              <Link key={teaser.href} href={teaser.href} className="group rounded-md border border-white/10 bg-surface p-7 transition-colors hover:border-accent/40">
            ...
            <div className="mt-14 divide-y divide-white/10 border-y border-white/10">
              {faqs.map(faq => (
                <details key={faq.q} className="group py-6">
                  <summary className="flex cursor-pointer list-none items-center justify-between gap-4 font-heading text-lg font-semibold marker:content-none">
                    {faq.q}
                    <ChevronDown className="shrink-0 text-accent transition-transform duration-200 group-open:rotate-180" size={18} />
                  </summary>
                  <p className="mt-4 max-w-2xl text-sm leading-6 text-muted-foreground">{faq.a}</p>
                </details>
              ))}
            </div>
          </div>
